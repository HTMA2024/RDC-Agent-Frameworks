...
| Claude Code | Yes | Yes | Yes | Yes | Alias-level | Prompt-directed | `concurrent_team` | Project config plus subagents |
...
| Claude Desktop | No | Yes (wrapper) | No | Yes | Inherit-only | Workflow brief only | `workflow_stage` | Desktop MCP config |
| Manus | Workflow-only | Yes (wrapper) | No | No | Inherit-only | Workflow-level | `workflow_stage` | Workflow package |
...
- `claude-code` 支持 per-agent routing，但可选模型族受宿主模型池限制；若宿主仍要求 session bootstrap agent，可保留 `team-lead` 作为内部 bootstrap/orchestrator，而 public user entry 统一是 `rdc-debugger`。
- `claude-desktop` 和 `manus` 属于 inherit-only 的降级宿主，但仍提供 wrapper skill 作为统一入口语义。
