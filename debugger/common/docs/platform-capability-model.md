# Platform Capability Model（平台能力模型）

本文件解释 `common/config/platform_capabilities.json` 的字段语义。  
`platform_capabilities.json` 是平台能力的唯一权威源；README、AGENTS、wrapper skill 和矩阵文档都只允许镜像它，不得另发明第二套口径。

## 1. 两层真相必须分开

平台能力必须拆成两层：

- 平台 agentic 能力
  - `coordination_mode`
  - `sub_agent_mode`
  - `peer_communication`
  - `agent_description_mode`
  - `dispatch_topology`
- runtime/backends 能力
  - `local_support`
  - `remote_support`
  - `supported_entry_modes`
  - `supported_backends`
  - `remote_coordination_mode`
  - `enforcement_layer`

Tools 只定义 transport/runtime ceiling；Frameworks 才定义平台如何利用这层 ceiling。

固定口径：

- sub agent 支持不等于 team agents
- staged_handoff 是 hub-and-spoke 多轮接力

## 2. Agentic 能力字段

### `sub_agent_mode`

- `team_agents`
  - 子 agent 之间可直接通信。
- `puppet_sub_agents`
  - 子 agent 可存在稳定 specialist 角色，但不能彼此直连；所有协调经主 agent 中转。
- `instruction_only_sub_agents`
  - 宿主不承载独立 agent 描述文件；需要子 agent 时，只能由主 agent 在实例化时注入 instruction。

### `peer_communication`

- `direct`
  - specialist 之间可直接交换上下文。
- `via_main_agent`
  - specialist 之间不直连；主 agent 是唯一通信与裁决中枢。
- `none`
  - 不建立稳定 specialist 网络，只允许阶段化 worker。

### `agent_description_mode`

- `independent_files`
  - 宿主支持独立 agent 文件或稳定的 per-agent 描述。
- `spawn_instruction_only`
  - 宿主不支持独立 agent 描述文件，只能在 spawn 时注入 instruction。

### `dispatch_topology`

- `mesh`
  - team agents 允许直接协作。
- `hub_and_spoke`
  - `staged_handoff` 的权威拓扑；主 agent 负责任务拆分、证据汇总、冲突裁决和下一轮 brief 重组。
- `workflow_serial`
  - `workflow_stage` 的权威拓扑；阶段化推进，可有临时 worker，但不形成稳定 specialist 网络。

## 3. 协作模式的精确定义

### `concurrent_team`

- 只适用于 `team_agents` 宿主。
- local 下可利用 multi-context runtime ceiling。
- remote 仍必须降级到单 owner。

### `staged_handoff`

- 不是单 agent 串行。
- 它是主 agent 中枢式的多 specialist 多轮接力。
- specialist 之间不能直连，但可以通过主 agent 的重新裁决形成 N+1 轮 brief、probe request 与 evidence 回填。

### `workflow_stage`

- 这是近似串行阶段流。
- 可以有临时 worker，但不承诺稳定 specialist 网络。
- `serial_only` remote 只能落在这一档。

## 4. Support 与 Enforcement

### `local_support`

- `verified`
- `degraded`
- `unsupported`

### `remote_support`

- `verified`
- `serial_only`
- `unsupported`

其中 `serial_only` 表示 remote 是正式支持路径，但只能按 `workflow_stage + single_runtime_owner` 串行推进。

### `enforcement_layer`

- `hooks`
  - 宿主可在 host-side 提前阻断。
- `runtime_owner`
  - 主要依赖 Tools 的 owner/context/baton surface 与 Frameworks 的 gate/audit。
- `audit_only`
  - 无法做 host-side 拦截，只能依赖显式 artifacts 与审计。

## 5. 平台默认矩阵解释

- `claude-code` / `code-buddy`
  - `concurrent_team + team_agents`
- `codex` / `cursor` / `copilot-cli` / `copilot-ide`
  - `staged_handoff + puppet_sub_agents`
- `manus` / `claude-desktop`
  - `workflow_stage + instruction_only_sub_agents`

所有平台都要遵守：

- `remote_coordination_mode = single_runtime_owner`
- `remote` 不允许多 live owners
- local 是否能真正并行，要看平台 agentic 能力是否允许利用 Tools 给出的 runtime ceiling
