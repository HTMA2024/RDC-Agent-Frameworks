...
当前文件是 Cursor IDE 宿主入口。Agent 的目标是使用 RenderDoc/RDC platform tools 调试 GPU 渲染问题。
...
只有在 `session_evidence.yaml`、`skeptic_signoff.yaml`、`action_chain.jsonl` 完整后，你才能产出 final report。
运行时工作区固定为：`../workspace`
