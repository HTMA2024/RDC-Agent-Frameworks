# Kiro Workspace Instructions（工作区约束）

当前目录是 Kiro 的 platform-local 模板。所有角色在进入 role-specific 行为前，都必须先服从本文件与共享 `common/` 约束。
...
- `kiro` 的 `local_support` / `remote_support` / `enforcement_layer` 以 `common/config/platform_capabilities.json` 当前行与 `runtime_mode_truth.snapshot.json` 为准
- 当前平台的 `coordination_mode = staged_handoff`，`sub_agent_mode = puppet_sub_agents`
- Kiro sub-agents 之间不具备 team-agent 直连能力；所有依赖、冲突与下一轮 brief 都经 `rdc-debugger` 中转
